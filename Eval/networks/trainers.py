...
import codecs as cs
from options.train_options import seed
    ...
    def train(self, train_dataset, val_dataset, plot_eval):
        ...
        while invalid:
            ...
            train_loader = DataLoader(train_dataset, batch_size=self.opt.batch_size, drop_last=True, num_workers=4,
                                      shuffle=True, collate_fn=collate_fn, pin_memory=True,worker_init_fn=seed_worker,generator=g,)
...
class TextMotionMatchTrainer(object):
    ...
    def resume(self, model_dir):
        ...
        self.opt_movement_encoder.load_state_dict(checkpoints['opt_movement_encoder'])
        return checkpoints['epoch'], checkpoints['iter']
    ...
    def save(self, model_dir, epoch, niter):
        state = {
            ...
            'opt_movement_encoder': self.opt_movement_encoder.state_dict(),
            'epoch': epoch,
            'iter': niter,
        }
        torch.save(state, model_dir)
    ...
    def update(self):
        ...
        self.zero_grad([self.opt_text_encoder,self.opt_motion_encoder,self.opt_movement_encoder])#self.zero_grad([self.opt_motion_encoder, self.opt_text_encoder])
        loss_logs = self.backward()
        self.loss.backward()
        self.clip_norm([self.text_encoder, self.motion_encoder,self.movement_encoder])#self.clip_norm([self.text_encoder, self.motion_encoder])
        self.step([self.opt_text_encoder,self.opt_motion_encoder,self.opt_movement_encoder])#self.step([self.opt_text_encoder, self.opt_motion_encoder])
