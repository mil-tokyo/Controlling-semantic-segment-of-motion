...
class Text2MotionDatasetV3(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, if_train):
        self.opt = opt
        ...
        with cs.open(split_file, 'r') as f:
            for line in f.readlines():
                id_list_pre.append(line.strip())
        len_traintxt=len(id_list_pre)
        splitpos=(len_traintxt*9)//10


        if if_train:
            id_list=id_list_pre[:splitpos]
        else:
            id_list=id_list_pre[splitpos:]
