...
from data.dataset2 import Text2MotionDataset_test, collate_fn
...
if __name__ == '__main__':
    ...
    if opt.dataset_name == 't2m':
        opt.data_root =  '../../../dataset/HumanML3D/HumanML3D'
    ...
    dataset = Text2MotionDataset_test(opt, mean, std, split_file, w_vectorizer)
    ...
    data_loader = DataLoader(dataset, batch_size=opt.batch_size, num_workers=1,
                             shuffle=False, collate_fn=collate_fn,worker_init_fn=seed_worker,generator=g,)
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            item_dict = {'caption': caption,
                        # 'length': m_lens[1],
                         'gt_motion': motions.squeeze(0).numpy()}
            print(caption)
            ...
            for t in range(opt.repeat_times):
                ...
                #, _, att_wgts = trainer.generate(word_emb, pos_ohot, cap_lens, m_lens, m_lens[0]//opt.unit_length, dim_pose,int2=True)
                ...
                sub_dict = {}
                sub_dict['motion'] = pred_motions.squeeze(0).cpu().numpy()
    ...
    for i, (key, item) in enumerate(result_dict.items()):
        ...
        sub_dict = item['result_00']
        motion = sub_dict['motion']
        endframenum=motion.shape[0]

        path_name='text20_generatadvideo/{:07d}/'.format(i)
        os.makedirs(path_name, exist_ok=True)
        path_txt=path_name+'story.txt'

        with open(path_txt, mode='w') as f:
            f.write(captions[0])
        np.savez(path_name+'data',people=motion,gen_framelen=endframenum,\
        gt=gt_motions,gt_framelen=framelen)
