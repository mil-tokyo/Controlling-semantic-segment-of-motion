...
class Text2MotionDataset2(torch.utils.data.Dataset):
    def __init__(self, split_file):
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        ...
        sort_index=np.load('../index_list.npz')['index_list']#('index_file.npz')['index_file']
        file_list = sorted(glob.glob('../Comparative/text20_generatadvideo/*/'))
        mean = np.load('../Comparative/checkpoints/t2m/Comp_v6_KLD01/meta/mean.npy')
        std = np.load('../Comparative/checkpoints/t2m/Comp_v6_KLD01/meta/std.npy')
        n_file_list=[]
        c=0
        for i in sort_index:
            if c<10:
                ...
                print(i)
            if len(file_list)>i:
                n_file_list.append(file_list[i])
            else:
                print("out of indez!!")
        ...
        c=0
        for i in name_list:
            motion2=np.load(n_file_list[c]+'/data.npz')['people']
            motion2=motion2*std+mean
            motion2=np.insert(motion2,motion2.shape[1],0,axis=1)
            motion2[-1][-1]=1
            data = self.data_dict[i]
            motion, m_length, text_list = motion2, len(motion2), data['text']
            text_data = random.choice(text_list)
            caption, tokens = text_data['caption'], text_data['tokens']
            self.datalen.append(len(motion2))
            self.labellen.append(len(tokens)+2)
            self.motion.append(torch.from_numpy(motion2))
            self.textstore.append(data['text'])
            c+=1
